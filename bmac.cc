 ...
 * $Id: bmac.cc,v 1.18 2005/12/10 17:57:13 liyuan Exp $
 *
...
//   See http://www.isi.edu/~weiye/pub/bmac_ton.pdf
                                                                                                                                                           
#include "wireless-phy.h"
#include "bmac.h"

static class MacBmacClass : public TclClass {
public:
       MacBmacClass() : TclClass("Mac/BMAC") {}
...
#ifdef JOURNAL_PAPER
void BmacUpdateNeighbTimer::expire(Event *e) {
        a_->handleUpdateNeighbTimer();
}
                                                                                                                                                            
void BmacAdaptiveListenTimer::expire(Event *e) {
        a_->handleAdaptiveListenTimer();
}
#endif

void BmacGeneTimer::expire(Event *e) {
	a_->handleGeneTimer();
}

void BmacRecvTimer::expire(Event *e) {
	stime_ = rtime_ = 0;
	a_->handleRecvTimer();
}

void BmacRecvTimer::sched(double time) {
	TimerHandler::sched(time);
	stime_ = Scheduler::instance().clock();
	rtime_ = time;
}

void BmacRecvTimer::resched(double time) {
	TimerHandler::resched(time);
	stime_ = Scheduler::instance().clock();
	rtime_ = time;
}

double BmacRecvTimer::timeToExpire() {
	return ((stime_ + rtime_) - Scheduler::instance().clock());
}

void BmacSendTimer::expire(Event *e) {
	a_->handleSendTimer();
}

void BmacNavTimer::expire(Event *e) {
	a_->handleNavTimer();
}

void BmacNeighNavTimer::sched(double time) {
	TimerHandler::sched(time);
	stime_ = Scheduler::instance().clock();
	rtime_ = time;
}

void BmacNeighNavTimer::expire(Event *e) {
	stime_ = rtime_ = 0;
	a_->handleNeighNavTimer();
}

double BmacNeighNavTimer::timeToExpire() {
	return ((stime_ + rtime_) - Scheduler::instance().clock());
}

void BmacCsTimer::expire(Event *e) {
	a_->handleCsTimer();
}

// if pending, cancel timer
void BmacCsTimer::checkToCancel() {
	if (status_ == TIMER_PENDING)
		cancel();
}

// void BmacChkSendTimer::expire(Event *e) {
//   a_->handleChkSendTimer();
// }

void BmacCounterTimer::sched(double time) {
	...
	// This is implemented to be in step with the counter used in actual bmac.
...
double BmacCounterTimer::timeToSleep() {
	return ((stime_ + tts_) - Scheduler::instance().clock()) ;
}

void BmacCounterTimer::expire(Event *e) {
	tts_ = stime_ = 0;
...
#ifdef JOURNAL_PAPER
BMAC::BMAC() : Mac(), mhUpdateNeighb_(this),mhNav_(this), mhNeighNav_(this), mhSend_(this), mhRecv_(this), mhGene_(this), mhCS_(this), mhAdap_(this), syncFlag_(0) {
        int i;
#else
BMAC::BMAC() : Mac(), mhNav_(this), mhNeighNav_(this), mhSend_(this), mhRecv_(this), mhGene_(this), mhCS_(this), syncFlag_(0) {
#endif
	state_ = IDLE;
	...
	*/
	...
	//durSyncPkt_ = ((SIZEOF_BMAC_SYNCPKT) * 12 + 18) / 1.0e4 ;

	durSyncPkt_ =  (PRE_PKT_BYTES + (SIZEOF_BMAC_SYNCPKT * ENCODE_RATIO)) * byte_tx_time_ + 1;
	durSyncPkt_ = CLKTICK2SEC(durSyncPkt_);

	//durDataPkt_ = ((SIZEOF_BMAC_DATAPKT) * 12 + 18) / 1.0e4 ;
	durDataPkt_ = (PRE_PKT_BYTES + (SIZEOF_BMAC_DATAPKT * ENCODE_RATIO)) * byte_tx_time_ + 1;
	durDataPkt_ = CLKTICK2SEC(durDataPkt_);

	//durCtrlPkt_ = ((SIZEOF_BMAC_CTRLPKT) * 12 + 18) / 1.0e4;
	durCtrlPkt_ = (PRE_PKT_BYTES + (SIZEOF_BMAC_CTRLPKT * ENCODE_RATIO)) * byte_tx_time_ + 1;
	...
	// initialize neighbour table
        for (i = 0; i < BMAC_MAX_NUM_NEIGHBORS; i++) {
        ...
        for (i = 0; i < BMAC_MAX_NUM_SCHEDULES; i++) {
...
//               	dutyCycle_ = BMAC_DUTY_CYCLE;
	...
	if (!syncFlag_)
		...
		for (int i=0; i< BMAC_MAX_NUM_SCHEDULES; i++) {
			mhCounter_[i] = new BmacCounterTimer(this, i);
		...
		if ( selfConfigFlag_ == 1) {
                ...
                //printf("BMAC_UPDATE_NEIGHB_PERIOD: ............node %d %d at %.6f\n", index_, BMAC_UPDATE_NEIGHB_PERIOD, Scheduler::instance().clock());
                mhUpdateNeighb_.sched(BMAC_UPDATE_NEIGHB_PERIOD);
                //dump();
#else
		mhGene_.sched(t);
#endif
...
void BMAC::setMySched(Packet *pkt) 
{
	...
	if (pkt == 0) { // freely choose my schedule
		...
		struct bmac_sync_frame *pf = (struct bmac_sync_frame *)pkt->access(hdr_mac::offset_);
...
int BMAC::command(int argc, const char*const* argv)
{
	if (argc == 3) {
...
// XXXX bmac handler functions

void BMAC::handleSendTimer() {
	assert(pktTx_);
  
	struct hdr_bmac *sh = HDR_BMAC(pktTx_);
...
void BMAC::handleRecvTimer() {
	assert(pktRx_);
  
	struct hdr_cmn *ch = HDR_CMN(pktRx_);
	struct hdr_bmac *sh = HDR_BMAC(pktRx_);
	...
	case SYNC_PKT:
		...
		break;
	default:
		fprintf(stderr, "Unknown bmac pkt type, %d\n", sh->type);
		break;
	}
...
void BMAC::handleGeneTimer() 
{
	...
	if (state_ == WAIT_CTS) {  // CTS timeout
		...
		if (numExtend_ < BMAC_EXTEND_LIMIT) { // extend time
			printf("BMAC %d: no ACK received. Extend Tx time.\n", index_);
			numExtend_++;
...
#endif
		assert(dataPkt_);
		struct hdr_bmac *mh = HDR_BMAC(dataPkt_);
...
void BMAC::handleCounterTimer(int id) {
	...
	sched_1:
		...
		if (schedTab_[id].txData == 1 &&
			...
			struct hdr_bmac *mh = (struct hdr_bmac *)dataPkt_->access(hdr_mac::offset_);
			if ((u_int32_t)mh->dstAddr == MAC_BROADCAST)
				howToSend_ = BCASTDATA;
			else
				howToSend_ = UNICAST;
...
#ifdef JOURNAL_PAPER
void BMAC::handleUpdateNeighbTimer() {
        //printf("BMAC::handleUpdateNeighbTimer: ............node %d at %.6f\n", index_, Scheduler::instance().clock());
...
void BMAC::handleAdaptiveListenTimer() {
...
void BMAC::recv(Packet *p, Handler *h) {
	...
	if (state_ == CR_SENSE) {
		...
		// cancels only if timer is pending; bmac could be in CR_SENSE with timer cancelled
...
void BMAC::discard(Packet *p, const char* why)
{
	hdr_cmn *ch = HDR_CMN(p);
	hdr_bmac *sh = HDR_BMAC(p);
...
int BMAC::drop_RTS(Packet *p, const char* why) 
{
	struct bmac_control_frame *cf = (bmac_control_frame *)p->access(hdr_mac::offset_);
...
int BMAC::drop_CTS(Packet *p, const char* why) 
{
	struct bmac_control_frame *cf = (bmac_control_frame *)p->access(hdr_mac::offset_);
...
int BMAC::drop_DATA(Packet *p, const char* why) 
{
	hdr_bmac *sh = HDR_BMAC(p);
...
int BMAC::drop_SYNC(Packet *p, const char* why) 
{
	drop(p, why);
	return 1;
}
...
void BMAC::checkMySched()
{
        ...
        if (schedTab_[0].numNodes == 1 && numSched_ > 1 && numNeighb_ > 0) {
                for (i = 1; i < BMAC_MAX_NUM_SCHEDULES; i++) {
...
void BMAC::update_schedTab_neighbList()
{
...
void BMAC::update_neighbList()
{
        ...
        checkMySched();
        mhUpdateNeighb_.resched(BMAC_UPDATE_NEIGHB_PERIOD);
}

void BMAC::check_schedFlag()
{
        int i;
...
void BMAC::handleDATA(Packet *p) {

	// internal handler for DATA packet
	struct hdr_cmn *ch = HDR_CMN(p);
	struct hdr_bmac * sh = HDR_BMAC(p);
...
#ifdef JOURNAL_PAPER
void BMAC::handleSYNC(Packet *p) 
{

	struct bmac_sync_frame *sf = (struct bmac_sync_frame *)p->access(hdr_mac::offset_);
	...
	state_ = IDLE;
      	...
      	nodeId = BMAC_MAX_NUM_NEIGHBORS;
      	schedId = BMAC_MAX_NUM_SCHEDULES;
      	for (i = 0; i < BMAC_MAX_NUM_NEIGHBORS; i++) {
      	...
      	if (nodeId < BMAC_MAX_NUM_NEIGHBORS) {
         	if (neighbList_[nodeId].state == sf->state) {
				...
				//printf("#############################################################\n");
		   		...
		   		mhCounter_[0]->sched(sf->sleepTime);
                   		...
                   		for (j = 0; j < BMAC_MAX_NUM_NEIGHBORS; j++) {
      	...
      	if (schedId == BMAC_MAX_NUM_SCHEDULES) {  // unknow schedule
        	flag =1;
         	// add an entry to the schedule table
         	if (numSched_ < BMAC_MAX_NUM_SCHEDULES){
            		for (i = 0; i < BMAC_MAX_NUM_SCHEDULES; i++) {
      	...
      	if (nodeId == BMAC_MAX_NUM_NEIGHBORS) {  // a new node
         	// didn't find an empty entry in schedule table, just drop the new node
         	if (schedId == BMAC_MAX_NUM_SCHEDULES) return;
         	// add it to my neighbor list
         	if (numNeighb_ < BMAC_MAX_NUM_NEIGHBORS){
            	for (i = 0; i < BMAC_MAX_NUM_NEIGHBORS; i++) {
	...
	schedTab_[schedId].numNodes--;
        if (schedTab_[schedId].numNodes == 0)
        	...
        	if (schedId == BMAC_MAX_NUM_SCHEDULES) {
...
//printf("node: %d ..............data sent............\n",index_);
	...
	int i, j;
	int foundNeighb = 0;
	int schedId = BMAC_MAX_NUM_SCHEDULES;
	//double t = Scheduler::instance().clock();
	//printf("Recvd SYNC (not/f) at %d from %d.....at %.6f\n", index_, sf->srcAddr, t);
...
bool BMAC::txNextFrag(void* data)
{
...
bool BMAC::sendMsg(Packet *pkt, Handler *h) {
	struct hdr_bmac *mh = HDR_BMAC(pkt);
#ifdef JOURNAL_PAPER
	...
	if ((u_int32_t)mh->dstAddr == MAC_BROADCAST) {
		return (bcastMsg(pkt));
                ...
                int fragNum = ch->size_ / SIZEOF_BMAC_DATAPKT ;
...
bool BMAC::unicastMsg(int numfrags, Packet *p) {
...
//printf("node: %d ..............data sent............\n",index_);
	...
	struct bmac_control_frame *cf = (struct bmac_control_frame *)p->access(hdr_mac::offset_);
	struct hdr_cmn *ch = HDR_CMN(p);

	ch->uid() = 0;
	ch->ptype() = PT_BMAC;
	ch->size() = SIZEOF_BMAC_CTRLPKT;
...
bool BMAC::sendCTS(double duration) {
...
bool BMAC::sendDATA() {
	...
	struct hdr_bmac * sh = HDR_BMAC(dataPkt_);
...
bool BMAC::sendACK(double duration) {
	// construct ACK pkt
	Packet *p = Packet::alloc();
	struct bmac_control_frame *cf = (struct bmac_control_frame *)p->access(hdr_mac::offset_);
	struct hdr_cmn *ch = HDR_CMN(p);
...
bool BMAC::sendSYNC()
{
  ...
  Packet *p = Packet::alloc();
  struct bmac_sync_frame *cf = (struct bmac_sync_frame *)p->access(hdr_mac::offset_);
  struct hdr_cmn *ch = HDR_CMN(p);

  ch->uid() = 0;
  ch->ptype() = PT_BMAC;
  ch->size() = SIZEOF_BMAC_SYNCPKT;
  ch->iface() = UNKN_IFACE.value();
  ch->direction() = hdr_cmn::DOWN;
  ch->error() = 0;	/* pkt not corrupt to start with */
  
  cf->length = SIZEOF_BMAC_SYNCPKT;
  cf->type = SYNC_PKT;
...
void BMAC::sentDATA(Packet *p)
{
  struct hdr_bmac *mh = HDR_BMAC(p);
...
void BMAC::updateNeighNav(double d ) {
  double now = Scheduler::instance().clock();
  //double d = duration * 1.0e-6; // convert to sec
...
double BMAC::txtime(Packet *p)
{
  ...
  default:
    fprintf(stderr, "invalid bmac pkt type %d\n", sh->type);
    exit(1);
  }
