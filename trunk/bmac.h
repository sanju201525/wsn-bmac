 ...
 * $Id: bmac.h,v 1.11 2005/08/25 18:58:07 johnh Exp $
 *
...
//  See http://www.isi.edu/scadds/papers/bmac_infocom.pdf for details
//
 ...
 * BMAC_MAX_NUM_NEIGHB: maximum number of neighbors.
 * BMAC_MAX_NUM_SCHED: maximum number of different schedules.
 * BMAC_DUTY_CYCLE: duty cycle in percentage. It controls the length of sleep 
 *   interval.
 * BMAC_RETRY_LIMIT: maximum number of RTS retries for sending a single message.
 * BMAC_EXTEND_LIMIT: maximum number of times to extend Tx time when ACK timeout
     happens.
 */

#ifndef BMAC_MAX_NUM_NEIGHBORS
#define BMAC_MAX_NUM_NEIGHBORS 20
#endif

#ifndef BMAC_MAX_NUM_SCHEDULES
#define BMAC_MAX_NUM_SCHEDULES 4
#endif

#ifndef BMAC_DUTY_CYCLE
#define BMAC_DUTY_CYCLE 10
#endif

#ifndef BMAC_RETRY_LIMIT
#define BMAC_RETRY_LIMIT 5
#endif

#ifndef BMAC_EXTEND_LIMIT
#define BMAC_EXTEND_LIMIT 5
#endif

#ifdef JOURNAL_PAPER

#ifndef BMAC_UPDATE_NEIGHB_PERIOD
#define BMAC_UPDATE_NEIGHB_PERIOD 50
#endif
...
/*  sizeof bmac datapkt hdr and bmac control and sync packets  */
...
#define SIZEOF_BMAC_DATAPKT 50  // hdr(10) + payload - fixed size pkts
#define SIZEOF_BMAC_CTRLPKT 10
#define SIZEOF_BMAC_SYNCPKT 9  


// Following are the ns definitions of the bmac frames
//SYNC PKT 
struct bmac_sync_frame { 
  int type; 
  int length; 
  int srcAddr;
  //int dstAddr;
...
// Timers used in bmac
class BmacTimer : public TimerHandler {
 public:
  BmacTimer(BMAC *a) : TimerHandler() {a_ = a; }
...
class BmacUpdateNeighbTimer : public BmacTimer {
 public:
  BmacUpdateNeighbTimer(BMAC *a) : BmacTimer(a) {}
  void expire(Event *e);
};
                                                                                                                                                            
// timer for putting nodes back to sleep after Adaptive Listen
class BmacAdaptiveListenTimer : public BmacTimer {
 public:
  BmacAdaptiveListenTimer(BMAC *a) : BmacTimer(a) {}
  void expire(Event *e);
};
#endif

// Generic timer used for sync, CTS and ACK timeouts
class BmacGeneTimer : public BmacTimer {
 public:
  BmacGeneTimer(BMAC *a) : BmacTimer(a) {}
  void expire(Event *e);
};

// Receive timer for receiving pkts
class BmacRecvTimer : public BmacTimer {
 public:
  BmacRecvTimer(BMAC *a) : BmacTimer(a) { stime_ = rtime_ = 0; }
...
// Send timer
class BmacSendTimer : public BmacTimer {
 public:
  BmacSendTimer(BMAC *a) : BmacTimer(a) {}
  void expire(Event *e);
};

// Nav- indicating if medium is busy or not
class BmacNavTimer : public BmacTimer {
 public:
  BmacNavTimer(BMAC *a) : BmacTimer(a) {}
  void expire(Event *e);
};

// Neighbor nav - if neighbor is busy or not
// used for data timeout
class BmacNeighNavTimer : public BmacTimer {
 public:
  BmacNeighNavTimer(BMAC *a) : BmacTimer(a) { stime_ = rtime_ = 0; }
  void sched(double duration);
  void expire(Event *e);
  double timeToExpire();
 protected:
...
// carrier sense timer
class BmacCsTimer : public BmacTimer {
 public:
  BmacCsTimer(BMAC *a) : BmacTimer(a) {}
  void expire(Event *e);
  void checkToCancel();
};

// synchronisation timer, regulates the sleep/wakeup cycles
class BmacCounterTimer : public BmacTimer { 
 public:  
  friend class BMAC;
  BmacCounterTimer(BMAC *a, int i) : BmacTimer(a) {index_ = i;}
  void sched(double t);
  void expire(Event *e); 
  double timeToSleep();
 protected:
...
// The bmac class
class BMAC : public Mac {
  
  friend class BmacGeneTimer;
  friend class BmacRecvTimer;
  friend class BmacSendTimer;
  friend class BmacNavTimer;
  friend class BmacNeighNavTimer;
  friend class BmacCsTimer; 
  friend class BmacCounterTimer;
#ifdef JOURNAL_PAPER
  friend class BmacUpdateNeighbTimer;
  friend class BmacAdaptiveListenTimer;
#endif
 ...
 private:
  ...
  // when bmac is not following SYNC (sleep-wakeup) cycles.
  ...
  // bmac methods to set dst, src and hdr_type in pkt hdrs
  inline int hdr_dst(char* hdr, int dst = -2) {
    struct hdr_bmac *sh = (struct hdr_bmac *) hdr;
    if (dst > -2)
      sh->dstAddr = dst;
    return sh->dstAddr;
  }
  ...
  // BMAC Timers
#ifdef JOURNAL_PAPER
  BmacUpdateNeighbTimer mhUpdateNeighb_; // timer for updating neighbors periodically
  BmacAdaptiveListenTimer mhAdap_; // timer for putting nodes back to sleep after adaptive listen
#endif
  BmacNavTimer	        mhNav_;		// NAV timer medium is free or not
  BmacNeighNavTimer     mhNeighNav_;    // neighbor NAV timer for data timeout
  BmacSendTimer		mhSend_;	// incoming packets
  BmacRecvTimer         mhRecv_;        // outgoing packets
  BmacGeneTimer         mhGene_;        // generic timer used sync/CTS/ACK timeout
  BmacCsTimer           mhCS_;          // carrier sense timer
  
  // array of countertimer, one for each schedule
  // counter tracking node's sleep/awake cycle
  BmacCounterTimer      *mhCounter_[BMAC_MAX_NUM_SCHEDULES];  
  ...
  struct SchedTable schedTab_[BMAC_MAX_NUM_SCHEDULES];   // schedule table
  struct NeighbList neighbList_[BMAC_MAX_NUM_NEIGHBORS]; // neighbor list
  ...
  int syncFlag_;  // is set to 1 when BMAC uses sleep-wakeup cycle
  int selfConfigFlag_;  // is set to 0 when BMAC uses user configurable schedule start time
